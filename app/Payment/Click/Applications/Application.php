...
namespace App\Payment\Click\Applications;

use App\Payment\Click\Utils\Helper;
use App\Payment\Click\Utils\Configs;
use App\Payment\Click\Exceptions\ClickException;
use Closure;
...
class Application
{
    ...
    /** @var $helper \App\Payment\Click\Utils\Helper object, it has included same helpfull methods */
    private $helper;

    /** @var $configs \App\Payment\Click\Utils\Configs object */
    private $configs;
    ...
    /**
     * @name \App\Payment\Click\Applications\Application::requestHandler method, it capable to detect the request type as router
     *
     * @param $model \App\Payment\Click\Models\Payments object
     * @return void
     */
    ...
    public function run()
    {
        if (isset($this->params['model'])) {
            ...
            if (!empty($provider_configs)) {
                // initalization click configs to model as provider
                $model->init_provider($provider_configs);
     ...
     * @param $func Closure, the session body
     *
     * @example:
     * Application::session('YOUR_AUTH_TOKEN', ['/prepare', '/complete'], function(){
     *      ...
