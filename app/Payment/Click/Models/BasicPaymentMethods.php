...
/**
 * @name \App\Payment\Click\Models\BasicPaymentMethods class, it has included the some basic Payments methods
 */
class BasicPaymentMethods extends BasicPaymentErrors
{
    /**
     * @name \App\Payment\Click\Models\BasicPaymentMethods::prepare the preapre request method
    ...
    /**
     * @name \App\Payment\Click\Models\BasicPaymentMethods::complete method, the complete request method
    ...
    public function complete($request = null)
    {
        ...
        if ($request['error'] < 0 && !in_array($result['error'], [-4, -9])) {
            ...
            // udpate store order
            $order = Order::findOrFail($request['merchant_trans_id']);
            $order->update([
